...
```kotlin
 SpeedDial(
    ...
    item {
        FabWithLabel(
            onClick = { showToast(context, "Direct image clicked!") },
            labelContent = { Text(text = "Direct Image PNG") },
            isDirectImage = true, // Use this to bypass the FAB circular background and display your own PNG
        ) {
            Image(painterResource(R.drawable.my_image), contentDescription = null, modifier = Modifier.fillMaxSize())
        }
    }
}
```
...
To display a full-size direct image (like a PNG) instead of a tinted icon inside a FAB shape, you can use `TYPE_DIRECT_IMAGE`:

```kotlin
speedDialView.addActionItem(
    SpeedDialActionItem.Builder(R.id.fab_direct_image, drawable)
        .setFabType(SpeedDialActionItem.TYPE_DIRECT_IMAGE)
        .setLabel(getString(R.string.label_direct_image))
        .create()
)
```
